...
def upgrade_state_dict_for_deltalm(
    state_dict: Dict[str, Any], pretrained_deltalm_checkpoint: str, is_encoder=True, is_retuned=False,
) -> Dict[str, Any]:
...
class DeltaLMDecoderLayer(TransformerDecoderLayerBase):
    ...
    def build_adapters(self, adapter_dim, input_dim):
        if not self.args.adapter:
            return None
        
        langs = self.args.adapter_groups.split(",") + self.args.adapter_families.split(",") + self.args.adapter_langs.split(",")
        return nn.ModuleDict({"dec" + k: Adapter(input_dim, adapter_dim) for k in langs})
    ...
    def forward(
        ...
        x = self.dropout_module(x)
        if self.adapters:
            langs = self.args.adapter_langs.split(",")
            hierarchies = self.args.adapter_hierarchies.split(";")
            if lang == None:
                lang = self.args.target_lang
            adapter_key = hierarchies[langs.index(lang)]
            group = "dec" + adapter_key.split(",")[0]
            family = "dec" + adapter_key.split(",")[1]
            lang = "dec" + adapter_key.split(",")[2]
            x = self.adapters[group](x, x)[0]
            if family != "decNA":
                x = self.adapters[family](x, x)[0]
            x = self.adapters[lang](x, x)[0]
        x = self.residual_connection(x, residual)
        if not self.normalize_before:
            x = self.final_layer_norm(x)
